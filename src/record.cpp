...
RecordID::RecordID (PageID pid, uint32_t slot_number) {
    page_id_ = pid;
...
Record::Record(char* data, uint32_t r_size, bool read_only): 
    data_(data),
    record_size_(r_size),
...
char* Record::getVariablePtr(uint32_t offset, uint16_t *size) {
...
char* Record::getFixedPtr(uint32_t offset){
    if( offset >= record_size_ ) return nullptr;
    return data_+offset;
}
