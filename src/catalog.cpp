    ...
    public:
            ...
            : cache_manager_ (cm)
        {
          ...
          fid_to_fname[meta_data_fid+1] = META_DATA_FSM;
            ...
            assert(tables_.count("NDB_INDEX_META") == tables_.count("NDB_INDEX_KEYS")); // both should either exist or not.
            if(tables_.count("NDB_INDEX_META")){
              load_indexes();
            } else {
              ...
              TableSchema* ret = createTable("NDB_INDEX_META", index_meta_columns); 
              assert(ret != nullptr);
              ...
              ret = createTable("NDB_INDEX_KEYS", index_keys_columns); 
              assert(ret != nullptr);
            }
        }
        ~Catalog() {
        ...
        bool createIndex(const std::string &table_name, const std::string& index_name, std::vector<std::string> &fields) {
          ...
          TableSchema* index_keys      = tables_["NDB_INDEX_KEYS"];
          ...
          for(int i = 0; i < cols.size(); ++i){
            ...
            std::vector<Value> vals;
            vals.emplace_back(Value(index_name));
            vals.emplace_back(Value(c));  // c is the offset inside of the table.
        ...
        bool load_indexes() {
          ...
          TableSchema* index_keys_schema = tables_["NDB_INDEX_KEYS"];
